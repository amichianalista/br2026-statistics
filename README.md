...
O app prioriza `SUPABASE_URL` com `SUPABASE_SERVICE_ROLE_KEY` ou `SUPABASE_SECRET_KEY` para ler as tabelas do schema `br_2026` pela API do Supabase. Se preferir, ele ainda aceita fallback por Postgres direto com `DATABASE_*` ou `DATABASE_URL`.
